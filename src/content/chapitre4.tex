...
Dès lors que les instructions au sujet du positionnement du dispositif ont été présentées aux participants, ils ont été invités à effectuer six allers-retours sur chaque type de sol, pour chaque position donnée. Il leur était indiqué, à la fois quand commencer et s’arrêter de marcher par un superviseur en charge de la gestion du \textit{wearable device} par l'intermédiaire de l’application \textit{Android}. Tout d'abord, il leur a été demandé de commencer par les graviers placés à l'intérieur du bac décrit précédemment. Dans un second temps, ils ont été invités à effectuer la même opération sur le sol, à côté du bac, considéré comme étant une sorte de ciment. Les participants ont ensuite été invités à sortir pour effectuer six autres allers et retours dans la neige. Afin de rester cohérents avec la procédure de l'expérimentation réaliser au sein du laboratoire, un ruban à mesurer respectant la longueur du bac a été placé sur la neige pour indiquer aux participants où faire demie tour. Enfin, puisque les graviers à l'intérieur du bac devaient être remplacés, les participants ont été recontactés pour effectuer l'expérimentation sur le sable ultérieurement. Cette même procédure a été répétée aussi bien avec la version 2 du \textit{wearable device} ($wear_v2$), ainsi qu'avec le téléphone intelligent ($cell$) à l'exception faite des enregistrements récoltés sur la neige, puisque la période de l'année durant laquelle cette deuxième partie de l'expérimentation a été conduite ne le permettait pas.
...
Afin de valider que la méthode de reconnaissance des types de sols proposée dans ce premier travail est suffisamment précise et fiable, il a été nécessaire de produire plusieurs jeux de données, dont la liste est fournie par le tableau \ref{tab:datasets}. En effet, les expérimentations faites par les participants ont seulement permis l'échantillonnage de données brutes à une fréquence stabilisée à $60\:Hz$. De plus, puisqu'un temps moyen de $6 s$ pour marcher un aller-retour dans le bac a été observé, c'est ce découpage qui a été utilisé pour le calcul des caractéristiques discriminantes, soit une fenêtre de taille $6\:s \times 60\:Hz = 600$ instances de données pour chaque participant, sur chaque type de sol et pour chaque position ce qui correspond alors à une donnée d'entrée pour l'algorithme d'apprentissage.
	...
	\begin{center}
		\resizebox{.9\textwidth}{!}{%
			\begin{tabular}{@{}rl@{}}
				...
				\textbf{soil\_type\_{[}\textit{wear\_v1}|\textit{wear\_v2}|\textit{cell}{]}\_{[}\textit{6}|\textit{9}|\textit{12}{]}} & \begin{tabular}[c]{@{}l@{}}Chaque instance est étiquettée \\[-15pt] avec le type de sol correspondant.\end{tabular} \\
				\textbf{soil\_type\_position\_{[}\textit{wear\_v1}|\textit{wear\_v2}|\textit{cell}{]}\_{[}\textit{6}|\textit{9}|\textit{12}{]}} & \begin{tabular}[c]{@{}l@{}}Chaque instance est étiquettée avec le type de sol \\[-15pt] correspondant et la position du wearable device.\end{tabular} \\ \bottomrule
			\end{tabular}%
		}
	\end{center}
\end{table}
...
% Since we have exploited several positions for the wearable device, we have created one dataset containing every soil types for each given place, being 5 distinct datasets in total. In that sense, we have been able to test the position-independent hypothesis.
