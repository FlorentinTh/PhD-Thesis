	...
	\begin{center}
		\resizebox{.9\textwidth}{!}{%
			\begin{tabular}{@{}rl@{}}
				...
				\textbf{soil\_type\_}\textbf{[ }\textit{wear\_v1}\textbf{ | }\textit{wear\_v2}\textbf{ | }\textit{cell}\textbf{ ]}\_\textbf{[ }\textit{6}\textbf{ | }\textit{9}\textbf{ | }\textit{12}\textbf{ ]} & \begin{tabular}[c]{@{}l@{}}Chaque instance est étiquetée \\[-15pt] avec le type de sol correspondant \\[-15pt] (\textit{p. ex.} sable).\end{tabular} \\
				\textbf{soil\_type\_position\_}\textbf{[ }\textit{wear\_v1}\textbf{ | }\textit{wear\_v2}\textbf{ | }\textit{cell}\textbf{ ]}\_\textbf{[ }\textit{6}\textbf{ | }\textit{9}\textbf{ | }\textit{12}\textbf{ ]} & \begin{tabular}[c]{@{}l@{}}Chaque instance est étiquetée avec le type de sol \\[-15pt] correspondant et la position du wearable device \\[-15pt] (\textit{p. ex.} poche\_droite\_sable).\end{tabular} \\ \bottomrule
			\end{tabular}%
		}
	\end{center}
\end{table}
...
\begin{figure}[ht!]
	...
	\subfloat[Les $F\mbox{-} mesures$ obtenues sur les deux ensembles de données avec différents paramètres pour l'algorithme \textit{random forest}.]{
		\includegraphics[width=.75\linewidth]{chapter4/results_wear_v1_rf.pdf}
	...
	\subfloat[Les $F\mbox{-} mesures$ obtenues sur les deux ensembles de données avec différents paramètres pour l'algorithme des $k$ plus proches voisins.]{
		\includegraphics[width=.75\linewidth]{chapter4/results_wear_v1_knn.pdf}
...
\begin{figure}[hb!]
	...
	\subfloat[Les $F\mbox{-} mesures$ obtenues pour l'évaluation de l'indépendance de positionnement avec l'algorithme \textit{random forest}.]{
		\includegraphics[width=.75\linewidth]{chapter4/pos_ind_rf_wear_1.pdf}
	...
	\subfloat[Les $F\mbox{-} mesures$ obtenues pour l'évaluation de l'indépendance de positionnement avec l'algorithme \acs{KNN}.]{
		\includegraphics[width=.75\linewidth]{chapter4/pos_ind_knn_wear_1.pdf}
		\label{fig:pos_ind_knn_wear_1}
    }
    \caption{Les $F\mbox{-} mesures$ obtenues par rapport aux valeurs références permettant l'évaluation de l'indépendance de positionnement du \textit{wearable device} dans sa version 1, pour les deux algorithmes : \textit{random forest} et \acs{KNN}.}
    \label{fig:pos_ind_wear_v1}
\end{figure}

Afin de s'assurer que la reconnaissance des sols soit fonctionnelle, et ce, indépendamment de la position choisie par l'utilisateur pour le porter, cinq jeux de données distincts ont été construits, soit un ensemble de données par emplacement. De ce fait, l'évaluation de la performance de reconnaissance a été réalisée avec les deux mêmes algorithmes d'apprentissage, configurés avec les paramètres optimaux identifiés dans l'expérimentation précédente. Les résultats obtenus sont données par les courbes continues représentées sur la figure \ref{fig:pos_ind_wear_v1}. De plus, afin de déterminer si la position du \textit{wearable device} avait un impact significatif sur le taux de reconnaissance, la moyenne des $F\mbox{-}mesures$ obtenues pour chaque position par rapport au type de sols ont été évaluées à l'aide du jeu de données \og \texttt{soil\_type\_position\_wear\_v1\_6} \fg. Ce sont les courbes discontinues également représentés sur la figure \ref{fig:pos_ind_wear_v1}. Les courbes constituent alors les valeurs de référence pour chaque position.

L'expression graphique donnée par ses différentes courbes permet de constater qu'aucune position distincte du \textit{wearable device} n'a un impact significatif sur la performance de la reconnaissance des types de sols. En effet, les différences maximum observés dans les $F\mbox{-}mesures$ sont de $2\%$ et $6\%$, respectivement avec l'algorithme \textit{random forest} et \acs{KNN}. De plus, il est possible, pour chaque algorithme d'observer une similarité entre la courbe exprimant les résultats et la courbe de référence, ce qui permet de confirmer la cohérence des résultats obtenus dans la vérification de l'hypothèse d'indépendance de position.

\subsubsection{\textit{Version 2 du wearable device}}

\begin{figure}[H]
	\centering
	\includegraphics[width=.75\linewidth]{chapter4/results_wear_v2.pdf}
        \caption{Résultats.}
...
\begin{figure}[H]
    \centering
	\subfloat[RF]{
		\includegraphics[width=.75\linewidth]{chapter4/pos_ind_rf_wear_2.pdf}
		\label{fig:pos_ind_rf_wear_2}
    }
    \\[20pt]
	\subfloat[KNN]{
		\includegraphics[width=.75\linewidth]{chapter4/pos_ind_knn_wear_2.pdf}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.75\linewidth]{chapter4/results_cell.pdf}
        \caption{Résultats.}
